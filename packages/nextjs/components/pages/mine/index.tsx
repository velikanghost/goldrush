...
import { useCountStore } from "~~/services/store/countStore";

const Mine = () => {
  const { tapTractor } = useCountStore();
  const { userMetrics, tractor } = useConnectStore();
      ...
      <div className="relative z-50 w-full rounded mx-auto bg-[#0a3d00]/50 border border-[#8cc63f] p-3 flex justify-between gap-1">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-1">
            ...
            {/* <Thunderbolt color="gold" /> */}
          </div>
          <progress
            className="progress progress-success bg-slate-600 border border-[#8cc63f]"
            value={(userMetrics.energy / userMetrics.max_energy) * 100}
            max="100"
          ></progress>
          <p className="text-sm text-primary-foreground font-headings">Energy Level</p>
        </div>
        <div className="flex flex-col items-end justify-between name font-headings text-primary-foreground">
          ...
          <p className="text-sm">
            Multiplier <span className="text-xl text-[#ffd700]">{tractor.multiplier}x</span>
          </p>
        </div>
      </div>
